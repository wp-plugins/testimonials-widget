...
class Testimonials_Widget_Widget extends WP_Widget {
	...
	public function form( $instance ) {
		...
		echo '<p><label for="'.$this->get_field_id( 'refresh_interval' ).'">'.__( 'Refresh Interval', 'testimonials-widget' ).' </label><input size="4" type="text" id="'.$this->get_field_id( 'refresh_interval' ).'" name="'.$this->get_field_name( 'refresh_interval' ).'" value="'.htmlspecialchars($instance['refresh_interval'], ENT_QUOTES).'" /><br/><span class="setting-description"><small>'.__( 'In seconds or 0 for no refresh', 'testimonials-widget' ).'</small></span></p>';
