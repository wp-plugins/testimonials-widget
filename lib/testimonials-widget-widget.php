...
class Testimonials_Widget_Widget extends WP_Widget {
	...
	public function update( $new_instance, $old_instance ) {
		...
		$instance['random'] = (isset($new_instance['random']) && $new_instance['random']) ? true : false;
	...
	public function form( $instance ) {
		...
		$show_author_checked = $show_source_checked	= $random_checked	= $tags_all_checked = '';
		...
		if($instance['random'])
			$random_checked = ' checked="checked"';
		...
		echo '<p><input type="checkbox" id="'.$this->get_field_id( 'random' ).'" name="'.$this->get_field_name( 'random' ).'" value="1"'.$random_checked.' /> <label for="'.$this->get_field_id( 'random' ).'">'.__( 'Random order', 'testimonials-widget' ).'</label><br/><span class="setting-description"><small>'.__( 'Unchecking this will rotate testimonials in the order added, latest first.', 'testimonials-widget' ).'</small></span></p>';
